...
class TestAffine:
    ...
    def test_exact_fit_rms_zero(self):
        m = gc.fit_geo_to_pixel_transform(self._affine_pts(), order=1)
        assert m["order"] == 1
        assert m["rms_px"] == pytest.approx(0.0, abs=1e-6)

    def test_project_reproduces(self):
        m = gc.fit_geo_to_pixel_transform(self._affine_pts(), order=1)
        px, py = gc.project_lonlat_to_pixel(m, 7, 4)
        assert px == pytest.approx(10 + 2 * 7)
        assert py == pytest.approx(5 + 3 * 4)

    def test_too_few_points_raises(self):
        with pytest.raises(ValueError):
            gc.fit_geo_to_pixel_transform([(0, 0, 0, 0), (1, 1, 1, 1)], order=1)   # 仅 2 点 < 3
...
class TestQuadratic:
    ...
    def test_quadratic_fits_curved(self):
        m = gc.fit_geo_to_pixel_transform(self._curved_pts(), order=2)
        assert m["order"] == 2
        assert m["rms_px"] == pytest.approx(0.0, abs=1e-6)
        px, py = gc.project_lonlat_to_pixel(m, 8, 8)
    ...
    def test_affine_worse_on_curved(self):
        ...
        m1 = gc.fit_geo_to_pixel_transform(pts, order=1)
        m2 = gc.fit_geo_to_pixel_transform(pts, order=2)
        assert m2["rms_px"] < m1["rms_px"]   # 二次更贴合弯曲

    def test_quadratic_too_few_raises(self):
        pts = [(i, i, i, i) for i in range(5)]   # 仅 5 点 < 6
        with pytest.raises(ValueError):
            gc.fit_geo_to_pixel_transform(pts, order=2)
...
class TestSerialize:
    def test_model_json_round_trip(self):
        ...
        m = gc.fit_geo_to_pixel_transform(pts, order=1)
        s = json.dumps(m)                 # 必须可 JSON 序列化（存 sidecar）
        m2 = json.loads(s)
        assert gc.project_lonlat_to_pixel(m2, 3, 3) == pytest.approx(gc.project_lonlat_to_pixel(m, 3, 3))

    def test_project_many_matches_scalar(self):
        pts = [(lon, lat, 10 + 2 * lon, 5 + 3 * lat)
               for lon, lat in [(0, 0), (10, 0), (0, 10), (5, 5)]]
        m = gc.fit_geo_to_pixel_transform(pts, order=1)
        xs, ys = gc.project_lonlat_to_pixels(m, [1, 2, 3], [4, 5, 6])
        assert list(xs) == pytest.approx([gc.project_lonlat_to_pixel(m, l, a)[0] for l, a in [(1, 4), (2, 5), (3, 6)]])
        assert list(ys) == pytest.approx([gc.project_lonlat_to_pixel(m, l, a)[1] for l, a in [(1, 4), (2, 5), (3, 6)]])
