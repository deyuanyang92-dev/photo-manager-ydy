"""Shared datatypes and helpers for the collaboration subsystem.

Split out of collab_service.py so the task store / HTTP API / network threads
can each live in a focused module. Compatibility imports remain available from
``app.services.collab_types`` while callers migrate incrementally.
"""
...
class PeerInfo:
    ...
    operator_name: str = ""       # display name chosen by the operator (广播给队友)
