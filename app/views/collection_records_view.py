...
_SECTIONS: list[tuple[str, list[tuple[str, str, str]]]] = [
    ...
    ("航次 / 平台", [
        ("cruise", "航次",      "潮下带船基；如 2026春季三门湾航次。潮间带可留空。"),
        ("vessel", "船号 / 船名", "潮下带船基采样的调查船，如 科学三号 / 浙渔科×号。潮间带可留空。"),
    ]),
    ("位置", [
        ("lon",        "经度",     "十进制度，留空存 NULL。"),
        ("lat",        "纬度",     "十进制度，留空存 NULL。"),
        ("depth",      "水深(m)",  "潮下带站位水深。潮间带可留空。"),
        ("geo_area",   "采集地理区", "如 浙江·三门湾。可由经纬度反解。"),
        ("water_body", "海区 / 水体", "DwC waterBody，如 东海 / 东海·三门湾。"),
    ]),
    ("采样", [
        ("sample_type",   "采集性质",   "定量 / 半定量 / 定性。定量需取样面积+次数（可算 个体数·m⁻²、生物量 g·m⁻²）；半定量如限时计数/CPUE；定性仅种类名单。"),
        ("method",        "采样方法",   "定量框 / 采泥器 / 拖网 / 手拣定性。"),
        ("sampler_model", "采泥器型号", "如 大洋50型 / Van Veen / 箱式采泥器 —— 型号/类型。"),
        ("sampler_spec",  "采样器规格", "尺寸 / 取样口面积，如 25×25cm框 / 0.1m²抓斗。"),
        ("sample_area",   "取样面积(m²)", "每站总取样面积，标准化 个体数·m⁻² / 生物量 g·m⁻² 的关键。"),
        ("replicates",    "取样次数",   "每站取的框数 / 采泥次数，如 泥滩8框、采泥4次。"),
        ("sieve_mesh",    "网筛孔径(mm)", "分选筛网孔径，大型底栖常 1.0。"),
        ("sample_no",     "样品编号",   "现场样品袋编号，如 B2-2026-007（DwC recordNumber）。"),
    ]),
    ("环境 / 分带", [
        ("tidal_zone",       "潮区",      "潮间带分带：高潮区 / 中潮区 / 低潮区。"),
        ("habitat",          "生境 / 底质", "泥滩 / 泥沙滩 / 沙滩 / 砾石滩 / 岩礁 …"),
...
# 新增（步骤 4）批量表格快速录入列。地区/样地 不在表里——它们由当前工作区（断面）继承，
# 见 project_settings_service.effective_new_specimen_prefill。
# 列序贴合 潮间带/潮下带 大型底栖动物 野外工作流；盐度/水温/天气/网筛/采样器规格 等
# 较恒定字段移到「逐条精修」+ Excel 模板，避免快速网格过宽。
