...
- **调查根项目**：一次完整调查，例如“广西采集项目-2026”。负责项目级元数据、人员、统一报告、跨子项目/断面汇总。
- **子项目/区域**：根项目下的业务分组，例如“广西儒艮2026”“广西白龙尾2026”。它可以只是容器，也可以被认领为可拍照工作区。
- **工作区**：一个可进入拍照的目录，通常是一个断面，例如“广西儒艮2026/断面C-201260613”，也可以是某个更细的站位、航次或学习子目录。
- **断面/站位组/学习子目录节点**：项目树上的业务子目录。它可以只是分类容器，也可以是工作区。
...
广西采集项目-2026/
  _data/
    project.db                   # 根项目数据库：项目元数据、工作区登记、报告索引、协作成员索引
    imports/
      stations_20260610.xlsx     # 原始导入留痕，可选
      collection_log_20260610.xlsx
    exports/
      广西采集项目-2026_标本汇总.xlsx
      广西采集项目-2026_采集事件汇总.xlsx
      广西采集项目-2026_照片资产清单.xlsx
      广西采集项目-2026_质控报告.html

  广西儒艮2026/                    # 子项目/区域容器，可继承根项目设置
    断面201260612/
      _data/
        project.db               # 工作区数据库：本站位/标本/照片/成果/协作任务事实
      incoming-jpg/
      results/
      reports/
    断面C-201260613/
...
- `report_runs`
- `workspace_meta`
- `project_catalog_service.register_workspace()`
- `enter_workspace(root=...)` 自动登记工作区
...
- `uid_sequences`
- `uid_reservations`
- `uid_sequence_service.reserve_block()/consume_next()`
- `photos`
- `photo_files`
- `photo_metadata`
- `photo_assignments`
...
- `audit_log`
- `qc_findings`
- `photo_asset_service.upsert_photo_file()/assign_photo()/mark_missing_files()`
- `monitor_service.scan_project()` 扫描 incoming JPG 时自动登记照片资产

已落地 M2b-基础：项目看板与责任审计底座。

- `label_print_events`
- `activity_audit_service.log_event()/record_label_print_event()`
- `project_summary_service.collect_project_dashboard()`
- 支持跨工作区统计：总标本、酒精/常规固定标本、RNA 标本、照片、未归属照片、标签打印次数。
- 支持按人员统计：拍摄人、采集人、鉴定人、照片归属操作人、标签打印人。

下一优先级 M2c：协作主机 API。

- 主机 API：申请编号、登记照片、创建/更新标本、照片归属、拉取工作区状态。
- 客户端策略：局域网在线时向主机申请编号；离线时只消费已预留号段。
- UI 明示：共享目录可以，多机不要直接写同一个 SQLite。

下一优先级 M2d：照片资产 UI + 项目看板 UI。

- 未归属照片列表。
- 照片 EXIF/metadata 详情。
- 按标本查看原始照片和成果。
- 缺失文件、重复文件、无 EXIF 的质控提示。
- 项目汇总页顶部显示项目看板卡片和人员分工表。

后续 M3：`stations + collection_events`。这会把经纬度和采集信息从“标本附属字段”提升为真正的野外数据模型。

后续 M4：项目级报告。等照片、工作区、采集事件稳定后，报告会自然变简单。
